...
def load(conn, icmdlist):
	...
	with conn.cursor() as cursor:
		...
		for cmd in icmdlist:
			# print (cmd)
			cursor.execute(cmd)
		command_pk = 'ALTER TABLE {TableName} ADD PRIMARY KEY (Year, CensusTract);'
        command_indx = 'CREATE INDEX idx_{TableName}_State ON {TableName}(State);'
        cursor.execute(command_pk)
        cursor.execute(command_indx)
